        ...
        if( $_POST["metodo"] == "PRODUCTO" )
        {
            if( $_POST["submetodo"] == "LISTARPRODUCTOS" )
            {
                ...
                foreach( $listarProductos as $key => $value )
                {
                    ...
                    $value['unidadMedida'] = $conexion->buscarVariosRegistro("SELECT idmedida FROM tb_productos_unidadesMedidas WHERE idproducto = ".intval($value['id_producto']));
                ...
                if( intval( $_POST['idproducto'] ) == 0)
                {
                    ...
                    if( $idcategoria > 0)
                    {
                        ...
                        if( intval( count( $buscarProducto ) ) == 0 )
                        {
                            ...
                            {
                                $idMedidas = $_POST['unidadesmedidas'];
                                $total = 0;
                                $idproducto = $conexion->lastId();
                                for( $i = 0; $i < count($idMedidas); $i++ )
                                {
                                    $guardarMedidaProducto = $conexion->ejecutar("INSERT INTO tb_productos_unidadesMedidas ( idproducto, idmedida ) VALUES (?,?)", array( $idproducto, intval( $idMedidas[$i] ) ) );
                                    if( $guardarMedidaProducto )
                                    {
                                        $total++;
                                    }
                                }
                                
                                if( $total == count($idMedidas) )
                                {
                    ...
                    if( $actualizarProducto )
                    {
                        ...
                        $deleteUnidadesMedidas = $conexion->ejecutar("DELETE FROM tb_productos_unidadesMedidas WHERE idproducto = ? ", array( intval($idproducto) ) );
                        if( $deleteUnidadesMedidas )
                        {
                            $total = 0;
